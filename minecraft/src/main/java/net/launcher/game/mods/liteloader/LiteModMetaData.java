...
{
	public static ModContainer<LiteModMetaData> createMod(LiteModMetaData data)
	{
		Objects.requireNonNull(data);
		DefaultArtifactVersion version = new DefaultArtifactVersion(data.getVersion());
		VersionRange range = VersionRange.createFromVersion(data.getMcVersion(), null);
		return new ModContainer<>(data, version, data.getName(), data.getAuthor(), data.getDescription(), range,
				ModTypes.LITE_LOADER);
	}
