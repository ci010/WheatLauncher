	...
	public void setupProfileOptionDialogs()
	{
		...
		ValidatorBase validatorBase = new ValidatorBase()
		{
			...
			protected void eval()
			{
				...
				else if (!text.equals(Core.INSTANCE.getCurrentProfile().nameProperty().get())
						&& Core.INSTANCE.getProfile(text) != null)
				{
		...
		acceptRename.setOnMouseClicked(event ->
		{
			if (renameProfileTextField.validate())
			{
				...
				Core.INSTANCE.getCurrentProfile().nameProperty().set(renameProfileTextField.getText());
			}
		});
		...
		acceptNewProfile.setOnMouseClicked(event ->
		{
			if (newProfileTextField.validate())
			{
				...
				Core.INSTANCE.newProfileAndSelect(newProfileTextField.getText());
			}
		});
	}
